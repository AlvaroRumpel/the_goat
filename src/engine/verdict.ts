import type { Award, Career, IconicMomentId, Tier, Verdict } from './types'
...
export function computeVerdict(career: Pick<Career, 'seasons' | 'fame'>): Verdict {
  ...
  for (const s of career.seasons) {
    ...
    for (const id of (s.iconicMoments ?? [])) iconicSum += ICONIC_VALUES[id]
    chokesSum += s.chokes ?? 0
  }
  const iconicPoints = Math.min(100, iconicSum)
  const loyalty =
  ...
  const score = Math.round(
    ...
    points / 400 + peakPpg * 2 + loyalty + career.fame * 0.5 + iconicPoints,
  )
  const totals = {
  ...
  return { score, tier, counts, totals, iconicPoints, chokes: chokesSum }
}
